...
class MPCController:
    def __init__(self, global_planed_traj, N=20, desired_v=0.3, v_max=0.4, w_max=0.4, ref_gap=4):
        ...
        # -> optimized
        self.update_ref_traj(global_planed_traj)
    ...
    def update_ref_traj(self, global_planed_traj):
        ...
        # -> optimized
        self.ref_cum_dist = np.cumsum(np.linalg.norm(np.diff(self.ref_traj, axis=0), axis=1))
    ...
    def find_reference_traj(self, x0, global_planed_traj):
        ...
        # -> optimized
        diff = global_planed_traj - x0[:2].reshape((1, 2))
        nearest_idx = np.argmin(np.einsum('ij,ij->i', diff, diff))
        ...
        for i in range(nearest_idx, len(global_planed_traj) - 1):
            ...
            # -> optimized
            if self.ref_cum_dist[i] - self.ref_cum_dist[nearest_idx] >= desire_arc_length * len(ref_traj_pts):
